...
def count_fully_containing_ranges(pair_assignment_file):
    count = 0
    for line in pair_assignment_file:
        ...
        if (
            (interval_a[0] >= interval_b[0] and interval_a[1] <= interval_b[1]) or
            (interval_b[0] >= interval_a[0] and interval_b[1] <= interval_a[1])
        ):
            count += 1
...
def main():
    ...
    with args.file.open('r', encoding='utf-8') as pair_assignment_file:
        count = count_fully_containing_ranges(pair_assignment_file)
